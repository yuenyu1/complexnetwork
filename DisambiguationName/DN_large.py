...
def changeBlocks(author, Blocks):
    author = author.upper()
    if ',' in author:
        surName, name = author.split(',', 1)
        try:
            tempKey = surName+','+name.replace(' ', '')
        except:
            tempKey = surName
    ...
    with open(inputFile, 'r') as f:
        ...
        for i in f.readlines():
            ...
            for author in AU.split('; '):
                ...
                tempKey = changeBlocks(author, Blocks)
                Blocks[tempKey].append(tempArray)
                N += 1
    ...
    for i in range(len(data) - 1):
        for j in range(i+1, len(data)):
            ...
            if len(set(data[i]['RI']) & set(data[j]['RI'])) >= 1:
                Edges.append((data[i]['AF'] + ';' + data[i]['UT'], data[j]['AF'] + ';' + data[j]['UT'], 1))
                G.add_edge(i, j)
                continue
...
##第二轮根据相似度进行连边
    ...
    for i in range(len(data) - 1):
        for j in range(i+1, len(data)):
            ...
            if data[i]['AF'] != data[i]['AU'] and data[j]['AF'] != data[j]['AU']:
                tempName = nameSimilarity2(data[i]['AF'], data[j]["AF"])
                if tempName == False:
            ...
            # else:
            #     tempName = nameSimilarity2(data[i]['AU'], data[j]['AU'])
            #     if tempName == False:
            #         Edges.append((data[i]['AF'] + ';' + data[i]['UT'], data[j]['AF'] + ';' + data[j]['UT'], 0))
            #         continue
            #     else:
            ...
            pFU = min(1, len(set(data[i]['FU']) & set(data[j]['FU'])) * 0.5)
...
def mainMethod(inputFilePath, outputFilePath):
    ...
    Blocks2 = {}
    for i in range(len(AuthorsNew)):
        print 2
        if AuthorsNew[i][0].upper() in Blocks2.keys():
            Blocks2[AuthorsNew[i][0].upper()] = Blocks2[AuthorsNew[i][0].upper()] + AuthorsNew[i][-1]
        else:
            Blocks2[AuthorsNew[i][0].upper()] = AuthorsNew[i][-1]
        AuthorsNew[i] = ''
...
if __name__ == '__main__':
    mainMethod(unicode('Physics, Condensed Matter_Merge.txt',"utf-8"), '/Users/yuenyu/Desktop')

    # f = open('Blocks', 'r')  # 读取生成文件blocks
    # Blocks = json.loads(f.read())
    # f.close()
    # A = []
    # divided(Blocks['Zhang,Y'.upper()], A)
    # for i in A:
    #     print i